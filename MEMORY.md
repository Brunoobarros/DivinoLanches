...
3. **Bairro para Entrega Livre (Resolvido):** O campo de Bairro na opção de entrega foi alterado de um menu de seleção (dropdown) para um campo de preenchimento de texto livre obrigatório. O sistema conta com validação (impede finalizar sem preencher) e cálculo inteligente de taxa (busca correspondências no cadastro de taxas ignorando maiúsculas/minúsculas e acentos, e caso não encontre, aplica a taxa padrão de "Outros").